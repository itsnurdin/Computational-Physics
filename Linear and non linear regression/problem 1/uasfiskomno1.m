...
a.Gunakan metoda regresi linier dan regresi non-linier untuk menentukan persamaan fungsi f(x) yang dapat dibentuk dari data eksperimen diatas.
b.Tentukan nilai r2 yang terbesar dari kedua metoda tersebut.
c.Tentukan nilai x untuk fungsi f(x) = 0.85 dari kedua metoda tersebut, kemudian beri kesimpulan dari hasil yang kalian peroleh.
d. Bandingkan persamaan fungsi f(x) yang kalian peroleh dengan persamaan fungsi: f(x)=x^2/((1+x^2 ) ). 

input:
uasfiskomno1 ([1 2 3 4 5], [0.5 0.8 0.9 0.941176 0.961538])
%}


function uasfiskomno1(x,y)
%------------------------------------
%UAS_Fisika-Komputasi_Nomor-1
%------------------------------------
...
fprintf('Koefisien determinasi regresi (R kuadrat) = %.5f\n',rPangkat);
fprintf('persamaan linear y=[%.5f].*(x.^[%.5f])\n',bPangkat,a2Pangkat);
fprintf('\n') 
...
fprintf('Koefisien determinasi regresi (R kuadrat) = %.5f\n',rSat);
fprintf('persamaan linear y=[%.5f].*(x/((([%.5f]*[%.5f])+x)))\n',bSat,bSat,a1Sat);
fprintf('\n') 
